...
function generateHomepage() {
    ...
    const finalPrice = localStorage.getItem('lastCalculatedFinalPrice') || "0";
    
    if (!rawData) {
        document.body.innerHTML = '<div style="color:white; text-align:center; padding: 50px;"><h1>No car data found.</h1><p>Please run Agent 1 and Agent 2 first.</p><a href="launcher.html" style="color:cyan;">Back to Dashboard</a></div>';
        return;
    }

    const data = JSON.parse(rawData);
    // Handle nested format if it came directly from API
    ...
    <div class="max-w-7xl mx-auto space-y-8">
        <header class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-sage-800/30 pb-8">
            ...
            <div class="flex flex-col items-start md:items-end gap-4 w-full md:w-auto">
                ...
                <div class="text-3xl md:text-4xl font-display font-bold text-white">${formattedPrice}</div>
            </div>
        </header>
